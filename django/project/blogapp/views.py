...
class BlogModify(APIView):
    ...
    def patch(self, request, pk=None, format=None):
        ...
        try:
            ...
            if blog.author != request.user:
                return Response({"msg":"Only the owner can update this post."}, status=status.HTTP_403_FORBIDDEN)
        except Post.DoesNotExist:
    ...
    def delete(self, request, pk=None, format=None):
        ...
        try:
            blog = Post.objects.get(pk=id)
            if blog.author != request.user:
                return Response({"msg": "Only the owner can delete this post."}, status=status.HTTP_403_FORBIDDEN)
            blog.delete()
